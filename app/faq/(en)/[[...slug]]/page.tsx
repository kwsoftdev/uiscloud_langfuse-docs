...
export default async function FaqPage({ params }: PageProps) {
  ...
  if (page) {
    const localeLinks: LocaleLink[] = [{ lang: "en", href: "/faq" }];
    if (faqKrSource.getPage(slug)) {
      const slugPath = slug.length > 0 ? `/${slug.join("/")}` : "";
      localeLinks.push({ lang: "ko", href: `/faq/kr${slugPath}` });
    }
...
export async function generateMetadata({
  ...
  const page = faqSource.getPage(slug);
  if (page) {
    const languages = faqKrSource.getPage(slug)
      ? buildLocalizedAlternates({
          slug,
          defaultLocale: "en",
          routes: { en: "/faq", "ko-KR": "/faq/kr" },
        })
      : undefined;
    return buildSectionMetadata(page, "faq", "FAQ", slug, { languages });
  }
